...
def test_parse_http_password():
    ...
    assert (
        parsed.path
        == "xxx:yyy!@actinia.mundialis.de/api/v3/resources/demouser/resource_id-1e904ec8-ad55-4eda-8f0d-440eb61d891a/baum.tif"
    )
    assert parsed.archive is None
    assert parsed.scheme == "https"
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="Checking behavior on posix, not win32"
)
...
def test_vsi_path_scheme():
    ...
    assert (
        _vsi_path(_ParsedPath("/foo.tif", "tests/data/files.zip", "zip"))
...
def test_path_as_vsi_scheme():
    """Correctly make a vsi path"""
    assert (
        _ParsedPath("/foo.tif", "tests/data/files.zip", "zip").as_vsi()
        == "/vsizip/tests/data/files.zip/foo.tif"
    )
...
def test_vsi_path_curl():
    ...
    assert (
        _vsi_path(_ParsedPath("example.com/foo.tif", None, "https"))
