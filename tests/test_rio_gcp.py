...
def test_feature_seq(runner):
    ...
    result = runner.invoke(main_group, ["gcps", "tests/data/white-gemini-iv.vrt"])
...
def test_feature_seq_indent_rs(runner):
    ...
    result = runner.invoke(
        main_group, ["gcps", "tests/data/white-gemini-iv.vrt", "--indent", "2", "--rs"]
    )
    assert result.exit_code == 0
...
def test_feature_precision(runner):
    ...
    result = runner.invoke(
        main_group,
        ["gcps", "tests/data/white-gemini-iv.vrt", "--projected", "--precision", "1"],
    )
    assert result.exit_code == 0
...
def test_collection_geographic_precision(runner):
    ...
    result = runner.invoke(
        main_group,
        ["gcps", "tests/data/white-gemini-iv.vrt", "--collection", "--projected"],
    )
    assert result.exit_code == 0
    assert '"FeatureCollection"' in result.output
