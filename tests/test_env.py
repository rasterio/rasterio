...
def test_session_nested_env_with_global_multi_threaded(monkeypatch, gdalenv, caplog):
    ...
    with rasterio.Env(session=session) as env_outer:
        ...
        with futures.ThreadPoolExecutor(max_workers=2) as executor:
            fpayloads = [executor.submit(reader) for _ in range(2)]
...
@pytest.mark.parametrize("version", [(1, 9, 1), (1, 9), "1.9.1", "1.9", "1.9a"])
...
def test_gdalversion_class_cmp__include_patch():
    assert GDALVersion.parse("3.12.1", include_patch=True) > GDALVersion.parse(
        "3.12", include_patch=True
    )
