...
from .common import gdal_version, credentials
...
def test_warped_vrt_nondefault_nodata(path_rgb_byte_tif):
    ...
    with (
        rasterio.open(path_rgb_byte_tif) as src,
        WarpedVRT(src, crs=DST_CRS, src_nodata=None, nodata=None) as vrt,
    ):
        assert vrt.dst_crs == CRS.from_string(DST_CRS)
...
def test_warped_vrt_msk_add_alpha(dsrec, caplog, path_rgb_msk_byte_tif):
    ...
    with (
        rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN="TRUE") as env,
        rasterio.open(path_rgb_msk_byte_tif) as src,
        WarpedVRT(src, crs=DST_CRS, add_alpha=True) as vrt,
    ):
        assert vrt.src_nodata is None
        assert vrt.dst_nodata is None
        assert vrt.count == 4
...
def test_warped_vrt_msk_nodata(path_rgb_msk_byte_tif):
    ...
    with (
        rasterio.open(path_rgb_msk_byte_tif) as src,
        WarpedVRT(src, crs=DST_CRS, nodata=0.0) as vrt,
    ):
        assert vrt.dst_crs == CRS.from_string(DST_CRS)
        assert vrt.src_nodata is None
        assert vrt.dst_nodata == 0.0
...
def test_warped_vrt_set_src_crs_default(path_rgb_byte_tif, tmpdir):
    ...
    with (
        rasterio.open(path_crs_unset) as src,
        WarpedVRT(src, src_crs=original_crs) as vrt,
    ):
...
def test_warp_extras(path_rgb_byte_tif):
    ...
    with (
        rasterio.open(path_rgb_byte_tif) as src,
        WarpedVRT(src, crs=DST_CRS, init_dest=255) as vrt,
    ):
        rgb = vrt.read()
...
def test_wrap_s3():
    ...
    with (
        rasterio.open(L8TIF) as src,
        WarpedVRT(src, crs=DST_CRS, src_nodata=0, nodata=0) as vrt,
    ):
...
def test_warped_vrt_nodata_read(path_rgb_byte_tif):
    ...
    with (
        rasterio.open(path_rgb_byte_tif) as src,
        WarpedVRT(src, crs=DST_CRS, src_nodata=0) as vrt,
    ):
...
def test_no_add_alpha_read(path_rgb_msk_byte_tif):
    ...
    with (
        rasterio.open(path_rgb_msk_byte_tif) as src,
        WarpedVRT(src, add_alpha=False) as vrt,
    ):
        assert vrt.count == 3
...
def test_warped_vrt_1band_add_alpha():
    """Add an alpha band to the VRT to access per-dataset mask of a source"""
    with (
        rasterio.open("tests/data/shade.tif") as src,
        WarpedVRT(src, add_alpha=True) as vrt,
    ):
        assert vrt.count == 2
        assert vrt.mask_flag_enums == (
            [MaskFlags.per_dataset, MaskFlags.alpha],
...
def test_warpedvrt_float32_preserve(data):
    ...
    with (
        rasterio.open("tests/data/float32.tif") as src,
        WarpedVRT(src, src_crs="EPSG:4326") as vrt,
    ):
...
def test_warpedvrt_float32_override(data):
    ...
    with (
        rasterio.open(float32file) as src,
        WarpedVRT(src, src_crs="EPSG:4326", dtype="float32") as vrt,
    ):
        assert src.dtypes == vrt.dtypes == ("float32",)
...
def test_warpedvrt_float32_override_nodata(data):
    ...
    with (
        rasterio.open(float32file) as src,
        WarpedVRT(src, src_crs="EPSG:4326", nodata=0.0001, dtype="float32") as vrt,
    ):
        assert src.dtypes == vrt.dtypes == ("float32",)
...
def test_warped_vrt_resizing():
    """Confirm fix of #1921"""
    with (
        rasterio.open("tests/data/RGB.byte.tif") as rgb,
        WarpedVRT(rgb, height=10, width=10) as vrt,
    ):
...
def test_warped_vrt_resizing_repro():
    ...
    with (
        rasterio.open("tests/data/RGB.byte.tif") as rgb,
        WarpedVRT(rgb, crs="EPSG:3857", height=10, width=10) as vrt,
    ):
        assert vrt.height == 10
        assert vrt.width == 10
...
def test_vrt_src_kept_alive(path_rgb_byte_tif):
    ...
    with (
        rasterio.open(path_rgb_byte_tif) as dst,
        WarpedVRT(dst, crs="EPSG:3857") as vrt,
    ):
...
def test_issue2086():
    ...
    with (
        rasterio.open("tests/data/white-gemini-iv.vrt") as src,
        WarpedVRT(src, crs=DST_CRS) as vrt,
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            115698.0,
            marks=pytest.mark.skipif(
                not gdal_version.at_least("3.11"),
...
def test_warpedvrt_gcps__width_height(affine_c_param, tmp_path):
    ...
    with rasterio.open(
        tiffname, mode="w", height=800, width=800, count=3, dtype=numpy.uint8
    ) as source:
...
def test_warpedvrt_rpcs__width_height():
    with (
        rasterio.open("tests/data/RGB.byte.rpc.vrt") as src,
        WarpedVRT(src, src_crs="EPSG:4326", width=10, height=10) as vrt,
    ):
        assert vrt.height == 10
        assert vrt.width == 10
